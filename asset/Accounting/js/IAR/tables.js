var initTables = function(scope, compile){
	...
	$('#all-table').on('click', '.delete-iar', function(index, value){
		var data = all_table.row(this.closest('tr')).data();
		$.post(base_url+"/gss/head/iar/post_delete_iar", {id: data.id, status:'completed'}).done(function(){
			location.reload();
		});
	});
	
	$('#completed-table').on('click', '.delete-iar', function(index, value){
		var data = completed_table.row(this.closest('tr')).data();
		$.post(base_url+"/gss/head/iar/post_delete_iar", {id: data.id, status:'completed'}).done(function(){
			location.reload();
		});
