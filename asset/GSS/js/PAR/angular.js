...
angular.module('myApp', [])
				...
				link: function(scope, element, attrs){
					...
					element.append($compile(
						"<tr ng-repeat='item in draft_inventory track by $index'>"+
							"<td>{{ item.distinction_no }}</td>"+
							"<td>{{ item.asset_name }}</td>"+
							"<td>{{ item.asset_description }}</td>"+
							"<td>{{ item.asset_part_name }}</td>"+
							"<td>{{ item.asset_part_description }}</td>"+
  ...
  //       	});			
		    ...
		    $scope.date_today = today.getFullYear() + '-' + m;
		    //$scope.par_date_today = today.getDate()+''+(today.getMonth()+1)+''+today.getFullYear();
		    $scope.par_date_today = today.getFullYear() + '-' + m;
		    $scope.par_date_today2 = today.getFullYear() + '-' + m + '-' + d;
		    $scope.last_par_id = 1;
